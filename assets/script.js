...
function saveScore() {
    ...
    if (initials === "") {
        ...
        for (let i = 0; i < userObject; i++) {
            var leaders = localStorage.getItem(userObject);
            var leaderRanks = JSON.parse(leaders);
            ranking = document.createElement('li');
            ranking.innerHTML = leaderRanks[i].user + " - " + leaderRanks[i].name;
